...
const MAX_AGE = '600'
const CORS_RESPONSE_HEADER_PREFIX = 'access-control-'
const ORIGIN_PATTERN_REGEX = /^(https?):\/\/([A-Za-z0-9.*-]+)(?::(\d{1,5}))?$/
const ORIGIN_VALUE_REGEX = /^(https?):\/\/([A-Za-z0-9.-]+)(?::(\d{1,5}))?$/
...
function matchesOriginPattern (origin, configuredOrigin) {
  if (!configuredOrigin.includes('*')) {
    return configuredOrigin === origin
  }

  const configuredMatch = configuredOrigin.match(ORIGIN_PATTERN_REGEX)
  const originMatch = origin.match(ORIGIN_VALUE_REGEX)
  if (!configuredMatch || !originMatch) return false

  const [, configuredProtocol, configuredHost, configuredPort] = configuredMatch
  const [, originProtocol, originHost, originPort] = originMatch
  if (configuredProtocol !== originProtocol || configuredPort !== originPort) {
    return false
  }

  const hostPattern = configuredHost
    .split('*')
    .map(escapeRegex)
    .join('[A-Za-z0-9-]+')
  return new RegExp(`^${hostPattern}$`, 'i').test(originHost)
}
...
function getAllowedOrigin (origin, corsAllowOrigin = getCorsAllowOriginEnv()) {
  ...
  if (rawConfig) {
    ...
    for (const item of list) {
      if (matchesOriginPattern(normalizedOrigin, item)) {
        return trimmedOrigin
      }
    }
  }
...
function withCorsHeaderGuard (response) {
  let guardedResponse
  guardedResponse = new Proxy(response, {
    get (target, property) {
      if (property === 'setHeader') {
        return (name, value) => {
          if (typeof name === 'string' && name.toLowerCase().startsWith(CORS_RESPONSE_HEADER_PREFIX)) {
            return guardedResponse
          }
          target.setHeader.call(target, name, value)
          return guardedResponse
        }
      }
